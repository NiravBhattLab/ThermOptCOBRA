...
if strcmp(MeM,'FASTCORE')
    ...
    for i=1:numel(contexts)
       ...
       m=fastcore(cons_mod,current_core,tol);
...
elseif strcmp(MeM,'SWIFTCORE')
    [RxnImp,~] = GiniReactionImportance(geneExpression,model,'FASTCORE',ut,lt,ThS,coreRxn);
    col = ismember(geneExpression.context,contexts);
    % picking the consistent reactions for the context specified
    RxnImp = RxnImp(cons_mod_rxn_id,col);    
    ...
    for i=1:numel(contexts)
       current_core = find(RxnImp(:,i));   
       m=swiftcore(cons_mod,current_core,ones(numel(cons_mod.rxns),1),tol,0);
       if exist('filename','var')
            % saving the model
            fn = strcat(filename,contexts{i});
            save(fn,'m');
...
elseif strcmp(MeM,'TOCS')
    ...
    for i=1:numel(contexts)
       ...
       m = ThermOptiCS(cons_mod,current_core,tol,TICs,Dir,0,30);
       if exist('filename','var')
            % saving the model
            fn = strcat(filename,contexts{i});
            save(fn,'m');
...
elseif strcmp(MeM,'iMAT')
    ...
    for i=1:numel(contexts)
       ...
       m=iMAT(cons_mod,current_core,1,9,tol,{});
       if exist('filename','var')
            % saving the model
            fn = strcat(filename,contexts{i});
            save(fn,'m');
...
elseif strcmp(MeM,'MBA')
    ...
    for i=1:numel(contexts)
       current_core = RxnImp(:,i);   
       m=MBA(cons_mod,cons_mod.rxns(current_core==1),cons_mod.rxns(current_core==2),tol);
       if exist('filename','var')
            % saving the model
            fn = strcat(filename,contexts{i});
            save(fn,'m');
...
elseif strcmp(MeM,'GIMME')
    ...
    for i=1:numel(contexts)
       current_core = RxnImp(:,i);   
       m=GIMME(cons_mod,current_core,0,obj_frac);
       if exist('filename','var')
            % saving the model
            fn = strcat(filename,contexts{i});
            save(fn,'m');
...
elseif strcmp(MeM,'INIT')
    ...
    for i=1:numel(contexts)
       current_core = RxnImp(:,i);   
       m = INIT(cons_mod,current_core,tol);
       if exist('filename','var')
            % saving the model
            fn = strcat(filename,contexts{i});
            save(fn,'m');
...
elseif strcmp(MeM,'mCADRE')
    ...
    for i=1:numel(contexts)
       current_core = RxnImp(:,i);   
       m = mCADRE(cons_mod,current_core,conf_scr,[],[],[],tol);
       if exist('filename','var')
            % saving the model
            fn = strcat(filename,contexts{i});
            save(fn,'m');
